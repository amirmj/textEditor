...
public class NearbyWords implements SpellingSuggest {
	...
	public void deletions(String s, List<String> currentList, boolean wordsOnly) {
		for (int index = 0; index < s.length(); index++) {
			...
			if (!currentList.contains(sb.toString()) && !s.equals(sb.toString()) && (!wordsOnly || dict.isWord(sb.toString()))
					&& s.length() != 0) {
				currentList.add(sb.toString());
			}
		}
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while (retList.size() <= numSuggestions && !queue.isEmpty() && visited.size() < THRESHOLD) {
			...
			List<String> generatedWords = distanceOne(removedString, true);
			for (String string : generatedWords) {
	...
	public static void main(String[] args) {
		...
		String word = "kangaro"; // Pass NearbyWords any
		Dictionary d = new DictionaryHashSet();
		DictionaryLoader.loadDictionary(d, "data/dict.txt");
		NearbyWords w = new NearbyWords(d);
		List<String> generatedWords = w.distanceOne(word, true);
