...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) {
		if (word1.isEmpty() || word2.isEmpty() || !nw.dict.isWord(word1) || !nw.dict.isWord(word2)) {
			return null;
		}
		...
		while (!queue.isEmpty() && !curr.getWord().equals(word2)) {
			curr = queue.remove(0);
			for (String string : nw.distanceOne(curr.getWord(), true)) {
				if (!visited.contains(string)) {
					visited.add(string);
					WPTreeNode child = curr.addChild(string);
					queue.add(child);
	...
	public static void main(String[] args) {
		...
		List<String> path = myTree.findPath("time", "theme");
